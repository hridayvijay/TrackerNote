...
import { User, EmailAuthProvider, GoogleAuthProvider, linkWithCredential, updatePassword, updateProfile } from "firebase/auth";
import { auth } from "../firebase";
import { getUserProfile, checkUsernameAvailable, updateUsername, saveUsername, UserProfile } from "../services";
import { ArrowLeft, User as UserIcon, Mail, Lock, Check, CheckCircle2, XCircle, Chrome, Shield, RefreshCw, AlertCircle } from "lucide-react";
...
export default function AccountPage({ onBack, user }: AccountPageProps) {
  ...
  const [usernameChangeAvailable, setUsernameChangeAvailable] = useState<boolean | null>(null);
  ...
  const [submittingChangeUsername, setSubmittingChangeUsername] = useState(false);
  ...
  useEffect(() => {
    if (!profile || newUsername === profile.username || newUsername.length < 3) {
  ...
  const handleLinkEmailPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLinkSuccess("Email and Password login successfully linked! See settings update.");
      ...
      if (err.code === "auth/email-already-in-use") {
        setLinkError("This email address is already registered to another account.");
      } else if (err.code === "auth/credential-already-in-use") {
        setLinkError("This email is already linked with another account.");
      } else {
        setLinkError(err.message || "An error occurred while linking your account.");
      }
    } finally {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === "auth/requires-recent-login") {
        setPasswordError("For security purposes, please log out and log back in to change your password.");
      } else {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 relative z-10 font-sans">
      <div className="flex items-center space-x-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Configure your authentication, username profile, and login methods</p>
        </div>
      </div>
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="glass-panel p-6 rounded-2xl flex flex-col items-center text-center">
            <div className="w-20 h-20 bg-gradient-to-br from-indigo-500 to-cyan-500 rounded-full flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-indigo-500/25 mb-4">
              {profile?.username?.charAt(0).toUpperCase() || user.displayName?.charAt(0).toUpperCase() || user.email?.charAt(0).toUpperCase() || "?"}
            </div>
            ...
            {loadingProfile ? (
              ...
              <>
                ...
                <div className="w-full pt-4 border-t border-slate-200 dark:border-slate-800/80 text-left space-y-3">
                  <span className="text-xs uppercase tracking-wider text-slate-400 font-bold block">Authed Methods</span>
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between text-xs font-semibold bg-white/40 dark:bg-slate-800/40 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
                      ...
                      {isGoogleLinked ? (
                        <span className="text-emerald-500 flex items-center"><Check className="w-3.5 h-3.5 mr-1" /> Linked</span>
                      ) : (
          ...
          {!isEmailLinked && !loadingProfile && (
            <div className="bg-amber-500/10 dark:bg-amber-500/5 border border-amber-500/20 text-amber-700 dark:text-amber-300 p-5 rounded-2xl flex items-start space-x-3 text-sm">
              ...
              <div>
                <span className="font-bold block mb-1">Backup Auth Recommended</span>
                <span>You currently only sign in with Google. Link an email & password credentials underneath to allow logging in through both credentials anytime.</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">This updates your master identity saved in Firestore across TrackerNote.</p>
            </div>
            ...
            {loadingProfile ? (
              ...
              <form onSubmit={handleChangeUsername} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      value={newUsername}
                      onChange={(e) => setNewUsername(cleanUsernameString(e.target.value))}
                    ...
                    <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center">
                      {checkingChangeUsername && <RefreshCw className="w-4 h-4 text-indigo-500 animate-spin" />}
                      {!checkingChangeUsername && usernameChangeAvailable === true && <CheckCircle2 className="w-4 h-4 text-emerald-500" />}
                      {!checkingChangeUsername && usernameChangeAvailable === false && <XCircle className="w-4 h-4 text-red-500" />}
                    </div>
                  </div>
                ...
                <button
                  type="submit"
                  disabled={submittingChangeUsername || usernameChangeAvailable === false || (profile && newUsername === profile.username)}
          ...
          {!isEmailLinked && (
            <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Unlock logging in with credentials by submitting your credentials below</p>
              </div>
              ...
              <form onSubmit={handleLinkEmailPassword} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Chosen Username</label>
                    <div className="relative">
                      <UserIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                      <input
                        type="text"
                        value={linkUsername}
                        onChange={(e) => setLinkUsername(cleanUsernameString(e.target.value))}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Secure Password</label>
                  <div className="relative">
          ...
          {isEmailLinked && (
            <div className="glass-panel p-6 sm:p-8 rounded-3xl space-y-6">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Submit a secure password update for email credentials</p>
              </div>
